...
function nearestCandleTime(value: string | number | undefined, candles: CandlePoint[]) {
  const lastTime = candles[candles.length - 1]?.time;
  if (!lastTime) return null;
  const requestedTime = toUnix(value) ?? lastTime;
  return candles.reduce((nearest, candle) =>
    Math.abs(candle.time - requestedTime) < Math.abs(nearest - requestedTime) ? candle.time : nearest,
  candles[0].time);
}
...
export default function ChartPage() {
  ...
  const positionSeriesRef = useRef<Map<string, { entry: any; tpZone: any; slZone: any; tpPriceLine: any; slPriceLine: any }>>(new Map());
  const fittedRef = useRef(false);
  const [timeframe, setTimeframe] = useState("M1");
  const [snapshot, setSnapshot] = useState<ChartSnapshot>({ candles: [], orders: [] });
  ...
  async function loadChart(silent = false) {
    ...
    try {
      const data = await api.chartData({ symbol: "XAUUSD", timeframe, count: 180 });
      setSnapshot({
  ...
  useEffect(() => {
    ...
    const positions = snapshot.orders.filter((order) =>
      String(order.status || "").toLowerCase() === "open" &&
      String(order.order_kind || "").toUpperCase() === "MARKET" &&
      Number(order.price ?? order.entry ?? 0) > 0,
    );
    const activeTickets = new Set(positions.map((position) => String(position.ticket)));
    positionSeriesRef.current.forEach((series, ticket) => {
      ...
      if (series.tpPriceLine) seriesRef.current.removePriceLine(series.tpPriceLine);
      if (series.slPriceLine) seriesRef.current.removePriceLine(series.slPriceLine);
    ...
    positions.forEach((position) => {
      ...
      let startTime = nearestCandleTime(position.opened_at || position.created_at, normalizedCandles);
      ...
      overlay.tpZone.applyOptions({ baseValue: { type: "price", price: entryPrice } });
      overlay.slZone.applyOptions({ baseValue: { type: "price", price: entryPrice } });
      const tpData = tpPrice > 0 ? [{ time: startTime, value: tpPrice }, { time: endTime, value: tpPrice }] : [];
      const slData = slPrice > 0 ? [{ time: startTime, value: slPrice }, { time: endTime, value: slPrice }] : [];
      ...
      <Card className="min-h-[680px]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-black text-slate-950">Live Order Chart</h3>
                <span
                  className={cx(
              ...
              <p className="text-sm text-slate-500">
                Candles update every two seconds. Activated positions show entry, TP, and SL levels.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <select
              ...
              <AppButton variant="soft" onClick={refreshChart} disabled={refreshing || loading}>
                <RefreshCcw className={cx("h-4 w-4", (refreshing || loading) && "animate-spin")} />
                {loading ? "Loading..." : refreshing ? "Refreshing..." : "Refresh"}
              </AppButton>
            </div>
          </div>
          <div ref={containerRef} className="mt-4 h-[580px] w-full rounded-3xl border border-slate-200 bg-slate-50" />
          {!snapshot.candles.length && !loading ? (
            <div className="mt-[-580px] flex h-[580px] items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white/60 backdrop-blur-[1px]">
              <p className="text-sm font-semibold text-slate-500">No candle data loaded yet.</p>
            </div>
          ) : null}
          <p className="mt-3 text-xs font-semibold text-slate-500">
            {snapshot.updated_at ? `Updated ${new Date(snapshot.updated_at).toLocaleString()}` : "Waiting for chart data..."}
          </p>
      </Card>
    </div>
  );
