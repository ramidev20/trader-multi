...
} from "../components/ui/ConsoleLogPanel";
...
const LEGACY_CONTROLLER_SETTINGS_KEY =
  "trader.remoteControl.controllerSettings";
...
function importLegacyReceiver() {
  try {
    const saved = JSON.parse(
      globalThis.localStorage?.getItem(LEGACY_CONTROLLER_SETTINGS_KEY) || "{}",
    );
...
function inferServerLevel(message: string): RemoteLogEntry["level"] {
  ...
  if (
    ...
    return "error";
  if (line.includes("disconnected") || line.includes("active connections: 0"))
    return "warning";
  if (line.includes("executed") || line.includes("authenticated successfully"))
    return "success";
  return "info";
}
    ...
    <div className="flex flex-wrap items-center gap-2 border-b border-slate-200">
      {[
        ...
        >
          ...
          {activeTab === tab.id ? (
            <span
              className={`absolute inset-x-3 bottom-0 h-0.5 rounded-full ${accentBar}`}
            />
          ) : null}
        </button>
      ))}
...
function TopologyBar({
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg border border-slate-200 bg-white px-5 py-4 shadow-sm sm:px-6">
      <div className="flex items-center gap-3">
        <span
          className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl border-[1.5px] ${meIconClass}`}
        >
  ...
  return (
    <div className="space-y-4 rounded-lg border border-blue-200 bg-blue-50/50 p-4">
      ...
      <RemoteField
        ...
        placeholder="Paste the token from the receiver PC"
      />
      {error ? (
        <p className="text-sm font-semibold text-rose-600">{error}</p>
      ) : null}
      <div className="flex gap-3">
        <RemoteButton tone="primary" onClick={submit}>
          <Save className="h-4 w-4" />
          Save
        </RemoteButton>
        <RemoteButton onClick={onCancel}>
          <X className="h-4 w-4" />
          Cancel
        </RemoteButton>
      </div>
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!receiver.enabled ? (
            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-slate-500">
              Excluded from trades
            </span>
          ) : null}
        </div>
        <p className="mt-1 truncate text-xs font-semibold text-slate-500">
          {receiver.url || "No URL set"}
        </p>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:shrink-0">
        ...
        >
          <span
            className={`absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200 ${receiver.enabled ? "translate-x-5" : "translate-x-0"}`}
          />
        </button>
        {online ? (
          <RemoteButton
            className="h-9 px-3"
            onClick={() => disconnectReceiver(receiver.id)}
          >
            Disconnect
          </RemoteButton>
        ) : (
          <RemoteButton
            ...
            tone="primary"
            disabled={connecting}
            onClick={() => connectReceiver(receiver.id).catch(() => {})}
          >
        ...
        <button
          type="button"
          onClick={onEdit}
          className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-900"
          aria-label={`Edit ${receiver.label}`}
        >
    ...
    <div className="flex">
      {setupSteps.map((step, index) => (
        <div key={step} className="relative flex-1 pr-4">
          ...
          <p className="max-w-[150px] text-[11.5px] leading-tight text-slate-500">
            {step}
          </p>
        </div>
      ))}
...
export default function RemoteControlPage() {
  const [role, setRole] = useState<RemoteRole | null>(() => {
    try {
      const savedRole = globalThis.localStorage?.getItem(
        REMOTE_ROLE_STORAGE_KEY,
      );
      return savedRole === "receiver" || savedRole === "controller"
        ? savedRole
        : null;
    } catch {
      return null;
  ...
  const [activeTab, setActiveTab] = useState<RemoteTab>(() => {
    try {
      return globalThis.localStorage?.getItem(REMOTE_TAB_STORAGE_KEY) === "logs"
        ? "logs"
        : "settings";
    } catch {
      return "settings";
  ...
  const onlineCount = useMemo(
    () => receivers.filter((r) => r.status.state === "online").length,
    [receivers],
  );
  const enabledCount = useMemo(
    () => receivers.filter((r) => r.enabled).length,
    [receivers],
  );
  const controllerErrorCount = useMemo(
    () => controllerLogs.filter((entry) => entry.level === "error").length,
    [controllerLogs],
  );
  const receiverErrorCount = useMemo(
    () => receiverLogs.filter((entry) => entry.level === "error").length,
    [receiverLogs],
  );
  ...
  useEffect(
    () => subscribeRemoteLogs((entries) => setControllerLogs(entries)),
    [],
  );
  ...
  useEffect(() => {
    ...
    async function loadReceiverLogs() {
      try {
        ...
        if (cancelled) return;
        const adapterLines = Array.isArray(runtime?.logs?.adapter)
          ? runtime.logs.adapter
          : [];
        const nextLogs: RemoteLogEntry[] = adapterLines
          ...
          .filter(
            (line) => typeof line === "string" && line.includes("[REMOTE]"),
          )
          .slice(-80)
          .map((line, index) => {
        ...
        setReceiverLogs((current) => {
          if (current.some((entry) => entry.id === "receiver-log-fetch-error"))
            return current;
          return [
            ...current,
            {
              ...
              message:
                "Receiver logs could not be refreshed. Existing entries are being kept.",
              level: "warning",
            },
          ];
        });
  ...
  async function copyToken() {
    ...
    try {
      ...
      setTokenCopied(true);
      setReceiverMessage(
        "Token copied. Paste it into the receiver's entry on the controller PC.",
      );
      setReceiverError("");
      window.setTimeout(() => setTokenCopied(false), 1200);
    } catch (error) {
      setReceiverError(
        `Could not copy the token: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async function copyUrl() {
    ...
    try {
      ...
      setReceiverError(
        `Could not copy the URL: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async function saveReceiverSetup() {
    ...
    try {
      ...
      setReceiverMessage(
        acceptTrades
          ? "Saved. This PC now accepts authenticated controller connections."
          : "Saved.",
      );
    } catch (error) {
        ...
        {roles.map((item, index) => {
          ...
          return (
            <motion.button
              ...
              className={`group relative overflow-hidden rounded-lg border p-4 text-left transition-colors duration-300 sm:p-5 ${
                selected
                  ? isReceiver
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${isReceiver ? "bg-teal-100 text-teal-700" : "bg-blue-100 text-blue-700"}`}
                >
                  <Icon className="h-[18px] w-[18px]" />
                </span>
                <div className="min-w-0 flex-1">
                  <p
                    className={`font-mono text-[10px] font-bold uppercase tracking-[0.1em] ${isReceiver ? "text-teal-700" : "text-blue-700"}`}
                  >
                    {item.eyebrow}
                  </p>
                ...
                <span
                  className={`grid h-[18px] w-[18px] shrink-0 place-items-center rounded-full border transition-all ${selected ? (isReceiver ? "border-teal-500 bg-teal-500 text-white" : "border-blue-600 bg-blue-600 text-white") : "border-slate-300 text-transparent"}`}
                >
      ...
      <AnimatePresence mode="wait">
        {!role ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-center text-sm font-semibold text-slate-500"
          >
          ...
          >
            ...
            />
            <div className="space-y-6 rounded-xl border border-teal-100 bg-white p-4 shadow-sm sm:p-5">
              <RemoteTabs
                role="receiver"
                activeTab={activeTab}
                onChange={setActiveTab}
                errorCount={receiverErrorCount}
              />

              {activeTab === "settings" ? (
                <>
                  <div
                    className={`flex items-center justify-between gap-4 rounded-lg border p-4 transition-colors ${acceptTrades ? "border-teal-200 bg-teal-50/70" : "border-slate-200 bg-slate-50"}`}
                  >
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-sm font-black text-slate-900">
                          Accept remote trades
                        </p>
                        <span
                          className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-wider ${acceptTrades ? "bg-teal-100 text-teal-700" : "bg-slate-200 text-slate-600"}`}
                        >
                          {acceptTrades ? "Enabled" : "Disabled"}
                        </span>
                      </div>
                      <p className="mt-1 text-xs leading-5 text-slate-500">
                        Authenticated controllers can connect once this is on.
                      </p>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      <span
                        className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform duration-200 ${acceptTrades ? "translate-x-5" : "translate-x-0"}`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="min-h-[46px]" aria-live="polite">
                    {receiverMessage ? (
                      <p className="rounded-xl border border-teal-200 bg-teal-50 p-3 text-sm font-semibold text-teal-700">
                        {receiverMessage}
                      </p>
                    ) : null}
                    {receiverError ? (
                ...
                <RemoteLogPanel
                  emptyText="No receiver events yet. Authentication attempts, disconnects, and command results will appear here."
          ...
          >
            ...
            />
            <div className="space-y-6 rounded-xl border border-blue-100 bg-white p-4 shadow-sm sm:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <RemoteTabs
                  role="controller"
                  activeTab={activeTab}
                  onChange={setActiveTab}
                  errorCount={controllerErrorCount}
                />
                <div className="flex items-center gap-2 self-start rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700 sm:ml-auto">
                  <Wifi
                    className={`h-4 w-4 ${onlineCount ? "text-teal-600" : "text-slate-400"}`}
                  />
                  {onlineCount}/{receivers.length} online · {enabledCount} in
                  trade broadcast
                </div>
              </div>
                ...
                <>
                  <div className="space-y-3">
                    {receivers.length === 0 && formState.mode === "closed" ? (
                      <div className="flex items-center gap-3.5 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-5 py-6">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Add the first trading PC to start mirroring trades
                            to it.
                          </p>
                        </div>
                      </div>
                      ...
                      receivers.map((receiver) =>
                        formState.mode === "edit" &&
                        formState.receiver?.id === receiver.id ? (
                          <ReceiverForm
                            key={receiver.id}
                            initial={receiver}
                            onCancel={() => setFormState({ mode: "closed" })}
                            onSaved={() => setFormState({ mode: "closed" })}
                          />
                        ) : (
                          <ReceiverRow
                            key={receiver.id}
                            receiver={receiver}
                            onEdit={() =>
                              setFormState({ mode: "edit", receiver })
                            }
                          />
                        ),
                      )
