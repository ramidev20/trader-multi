...
export default function ScalpingPage() {
  ...
  }, [demandPrice, supplyPrice, minSlPips, liquiditySlPips, orderKind, riskPercent]);
  ...
  function buildPayload(side, price) {
    return {
      symbol: SYMBOL,
  ...
  async function handleStart() {
    ...
    const candidates = [
      { side: "demand", price: demandPrice, active: demandActive },
      { side: "supply", price: supplyPrice, active: supplyActive },
    ].filter((candidate) => !candidate.active && Number(candidate.price) > 0);

    if (!candidates.length) {
      setErrorText("Enter a valid demand and/or supply M15 amount to arm a search.");
      return;
    }
    ...
    try {
      const results = await Promise.allSettled(
        candidates.map(({ side, price }) => api.startZoneStrategy(buildPayload(side, price))),
      );
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          ...
          either one, the engine watches new M5 candles for the opposite zone, then new
          M1 candles for a zone matching the original side, before opening that trade.
          The demand and supply searches run independently, so both can be armed and
          both can fire. Lines draw live on the Chart tab.
        </p>
      </div>
